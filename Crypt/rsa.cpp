 ...
 *
 */
int rsa_pkcs1_decrypt_tachograph( rsa_context *ctx,
                       int mode, int *olen,
                       const unsigned char *input,
                       unsigned char *output,
                       int length)
...
//    unsigned char *p;
    ...
    ret = ( mode == RSA_PUBLIC )
          ? rsa_public(  ctx, input, output )
          : rsa_private( ctx, input, output );
...
int rsa_self_test2( int verbose )
{
	int len;
	rsa_context rsa;
	unsigned char sha1sum[20];
